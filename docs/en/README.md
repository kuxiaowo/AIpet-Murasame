...
Start the instance and copy the SSH login command, for example `ssh -p 12345 root@connect.cqa1.seetacloud.com`.
...
Run `run.py` with the default Python environment. Python >= 3.10 is required; if errors occur, try Python 3.10.
...
```bash
conda activate AIpet_env  # Example name; you can use another one
python run.py
```
...
<summary>V1.2.0 Notes (click to expand)</summary>
  ...
  Install Anaconda to configure the environment: [Anaconda official website](https://www.anaconda.com/download). Other virtual environment tools are also fine.
  ...
  Extract the downloaded project, then put the entire `GPT-SoVITS` folder into the `AIpet-Murasame` directory, alongside folders such as `tool` and `classes`.
...
### 5. `start_ai_pet.bat` exits immediately (V1.2.0)
...
### 6. Script garbled text or immediate exit (V1.2.0)
...
| Status | Task |
|:--:|:--|
| Done | Long-term memory feature |
| Done | Write desktop pet display size into the configuration file |
| Done | Add Q&A support and resolve some common issues |
| Done | One-click Python script startup with better compatibility and optimized dependencies, avoiding forced PyTorch downloads |
| Done | Change clothes |
|  | Complete and improve logging |
|  | Check the "always-on-top logic" to ensure the window always stays on top during games |
| Done | Try deploying TTS to Alibaba Cloud |
| Done | Switch all models to the Qwen series |
