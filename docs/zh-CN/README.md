...
首次启动后，选择后端与模型，填写用户名称，并通过切换界面语言分别检查中、
英文人格提示词后保存。运行时会自动选择对应语言的人格、模型指令、屏幕分析
提示词和显示文本。语音识别语言可独立设置为跟随界面、自动检测、常用语言，
也可以填写 Whisper 语言代码。视觉、TTS 和语音输入属于可选功能，相关依赖
尚未准备好时请先保持关闭。
...
| 语音输入 | 启用语音输入，选择麦克风、计算设备、faster-whisper 模型和识别语言。识别语言可以跟随界面、自动检测、选择常用语言或填写 Whisper 语言代码。长按 Caps Lock 两秒开始录音，松开后识别并发送。 |
...
本项目原创贡献的版权人包括 kuxiaowo（2025–2026）、fsh114514（2026）及其他贡献者。
版权范围、下游再发布说明及可直接使用的署名样例见
[《版权与下游署名》](../../COPYRIGHT.md)。
