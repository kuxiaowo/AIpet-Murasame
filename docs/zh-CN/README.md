...
- 在界面中直接创建、导入或修改人格提示词。
- 模型只返回经过验证的中文、日语、情绪和 `a`/`b` 立绘姿态，不会直接生成图层编号。
- 模型可根据语气在两套立绘间切换，姿态与情绪仍由程序映射为固定图层，更换模型也不会随机“拆脸”。
...
启用语音输入后，可以分别选择 faster-whisper 模型或仓库 ID，以及该模型的下载目录。下载和加载都严格使用填写的目录；目录为空时会提示先选择，不再静默下载到项目默认目录。下载进度、大小和当前文件直接显示在语音设置卡片中，关闭设置后下载仍会继续。
...
- **自动行为**：设置空闲提醒、历史长度和可持久化的勿扰模式，也可以在确认后立即清除全部历史对话。
...
临时语音和截图仍位于 `%LOCALAPPDATA%\AIpet-Murasame\cache`；Whisper 和 TTS 的新下载位置由用户在设置页分别指定。屏幕感知默认关闭；启用后，截图只会发送给当前配置的视觉模型。
