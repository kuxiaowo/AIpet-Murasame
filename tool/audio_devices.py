...
def _preferred_audio_input_devices(
    ...
    candidates = [
        ...
        if not _is_default_device_alias(device.name)
    ]
    ...
    ordered = sorted(
        candidates,
        key=lambda item: (
            hostapi_order.get(item.hostapi, 99),
            item.name.casefold(),
            item.hostapi.casefold(),
            item.index,
        ),
    )
    unique: dict[str, AudioInputDevice] = {}
    for device in ordered:
        unique.setdefault(_canonical_device_name(device.name), device)
    return sorted(
        unique.values(),
...
def _is_default_device_alias(name: str) -> bool:
    normalized = _canonical_device_name(name)
    aliases = (
