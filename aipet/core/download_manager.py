"""Shared managed-model download orchestration."""
...
TTS_ENGINE_ARCHIVE_NVIDIA50 = next(
    (
        archive.filename
        for archive in _PLATFORM_TTS_ARCHIVES
        if "nvidia50" in archive.filename.casefold()
    ),
    TTS_ENGINE_ARCHIVE,
)
TTS_ENGINE_ARCHIVES = {
    archive.filename: (archive.size, archive.sha256)
    for archive in _PLATFORM_TTS_ARCHIVES
}
BUNDLED_7ZIP = (
    PROJECT_ROOT
...
class AssetDownloadWorker(QThread):
    ...
    def _download_file(
        ...
        for index, url in enumerate(urls):
            try:
                return self._download_http_file(
                    item,
                    url,
                    completed_before,
                    total,
                ...
                errors.append(exc)
                if self._cancelled.is_set() or index == len(urls) - 1:
                    raise
                logger.warning(
                    "下载源失败，切换备用源 | %s | %s | %s",
                    item.relative_path,
...
def _tts_files(*, include_engine: bool = False) -> list[RemoteFile]:
    ...
    if include_engine:
        archive = get_platform_runtime().archives.select_tts_engine_archive(
            detect_nvidia_gpu_names()
        )
        files.append(
            RemoteFile(
                url="",
...
def select_tts_engine_archive(gpu_names: list[str] | None = None) -> str:
    ...
    return (
        ...
        .archives.select_tts_engine_archive(names)
        .filename
    )
...
def detect_nvidia_gpu_names() -> list[str]:
    ...
    try:
        result = subprocess.run(
            ...
            text=True,
            timeout=10,
            **get_platform_runtime().processes.hidden_subprocess_options(),
        )
...
def _extract_gpt_sovits_archive(
    ...
    try:
        ...
        if seven_zip is not None:
            ...
            raise RuntimeError(
                "7-Zip or tar/bsdtar is required to extract "
...
def _find_7zip() -> Path | None:
    candidates = get_platform_runtime().archives.seven_zip_candidates(
        PROJECT_ROOT,
        BUNDLED_7ZIP,
    )
...
def _extract_with_bsdtar(
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to start tar/bsdtar: {exc}"
        ) from exc
...
def _hidden_process_options() -> tuple[object | None, int]:
    options = get_platform_runtime().processes.hidden_subprocess_options()
    return (
        options.get("startupinfo"),
...
def _whisper_files(repository: str) -> list[RemoteFile]:
    ...
    for endpoint in (
        ...
        try:
            response = requests.get(
                f"{endpoint}/api/models/{repository}",
                params={"blobs": "true"},
                timeout=(15, 45),
            )
            response.raise_for_status()
            ...
            logger.warning(
                "Whisper 模型元数据请求失败 | %s | %s",
                endpoint,
                exc,
            )
    if payload is None:
        raise RuntimeError(
            "Unable to load faster-whisper model metadata: "
            + "; ".join(errors)
        )
    patterns = (
        "config.json",
    ...
    for sibling in payload.get("siblings", []):
        ...
        files.append(
            RemoteFile(
                url=(
                    f"{HUGGING_FACE_ENDPOINT}/{repository}/resolve/main/"
                    f"{quote(name)}"
                ),
                relative_path=name,
                size=size,
