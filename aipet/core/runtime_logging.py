"""Shared application and request logging."""
...
from aipet.core.config import get_user_data_dir
...
LOG_VIEWER = (
    PROJECT_ROOT / "aipet" / "platforms" / "windows" / "log_viewer.py"
)
...
def _ensure_viewer() -> bool:
    ...
    if not runtime.capabilities.log_viewer:
        return False
    ...
    try:
        _viewer_process = subprocess.Popen(
            ...
            close_fds=True,
            **runtime.processes.new_console_subprocess_options(),
        )
    except OSError:
...
def _viewer_command(
    ...
    return get_platform_runtime().processes.log_viewer_command(
        sys.executable,
        log_directory,
        parent_process_id,
        frozen=bool(getattr(sys, "frozen", False)),
        viewer_script=LOG_VIEWER,
    )


def _console_python_executable(executable: str) -> str:
    """Compatibility hook delegated to the active platform process policy."""

    return get_platform_runtime().processes.console_python_executable(
        executable
    )
